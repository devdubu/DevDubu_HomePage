    ...
    <nav class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
      ...
      <PopoverGroup class="hidden lg:flex lg:gap-x-12">
        ...
        <Popover class="relative">
          ...
          <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-1">
            <PopoverPanel class="absolute -left-8 top-full z-10 mt-3 w-96 rounded-3xl bg-white p-4 shadow-lg ring-1 ring-gray-900/5">
              <div v-for="item in company" :key="item.name" class="relative rounded-lg p-4 hover:bg-gray-50">
                <a :href="item.href" class="block text-sm font-semibold leading-6 text-gray-900">
                  {{ item.name }}
                  <span class="absolute inset-0" />
                </a>
    ...
    <Dialog class="lg:hidden" @close="mobileMenuOpen = false" :open="mobileMenuOpen">
      ...
      <DialogPanel class="fixed inset-y-0 right-0 z-10 flex w-full flex-col justify-between overflow-y-auto bg-white sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
          ...
          <div class="mt-6 flow-root">
            <div class="-my-6 divide-y divide-gray-500/10">
              <div class="space-y-2 py-6">
                <a v-for="item in products" :key="item.name" :href="item.href" class="group -mx-3 flex items-center gap-x-6 rounded-lg p-3 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50">
              ...
              <div class="space-y-2 py-6">
                ...
                <a v-for="item in company" :key="item.name" :href="item.href" class="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50">{{ item.name }}</a>
              </div>
              <div class="py-6">
        ...
        <div class="sticky bottom-0 grid grid-cols-2 divide-x divide-gray-900/5 bg-gray-50 text-center">
          <a v-for="item in callsToAction" :key="item.name" :href="item.href" class="p-3 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-100">{{ item.name }}</a>
        </div>
      </DialogPanel>
    </Dialog>
  </header>
